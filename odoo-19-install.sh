#!/bin/bash
################################################################################
# Instalación CORE de Odoo 19 (sin crear instancias/servicios)
# - Muestra progreso en consola (tee) y guarda logs
...
log_message() {
  local message=$1
  echo "$(date '+%F %T') - $message" | tee -a "$INSTALL_LOG_FILE"
}
handle_error() {
  local error_message=$1
  echo "$(date '+%F %T') - ERROR: $error_message" | tee -a "$ERROR_LOG_FILE"
}
...
# Usuario de BD para administrar (no es instancia)
...
if [ "$INSTALL_WKHTMLTOPDF" = "True" ]; then
  log_message "Instalando wkhtmltopdf (0.12.6.1-3 con Qt parcheado si aplica)..."
  if [ -n "$WKHTML_DEB" ]; then
    pushd /tmp >/dev/null
    wget -q "${WKHTML_BASE_URL}/${WKHTML_DEB}"
    sudo apt-get install -y "./${WKHTML_DEB}" 2>&1 | tee -a "$INSTALL_LOG_FILE"
    rm -f "${WKHTML_DEB}"
    popd >/dev/null
  else
    echo "Distro no jammy/noble: instalando desde repos (puede no traer Qt parcheado)." | tee -a "$INSTALL_LOG_FILE"
...
sudo -u "$OE_USER" "$OE_HOME/venv/bin/pip" install -r "https://raw.githubusercontent.com/odoo/odoo/${OE_VERSION}/requirements.txt" 2>&1 | tee -a "$INSTALL_LOG_FILE"
...
if [ "$IS_ENTERPRISE" = "True" ]; then
  ...
  if [ ! -d "$OE_HOME/enterprise/addons/.git" ]; then
    ...
    if [ $RET -ne 0 ]; then
      ...
      sudo -u "$OE_USER" "$OE_HOME/venv/bin/pip" install num2words ofxparse dbfread firebase_admin pyOpenSSL pdfminer.six 2>&1 | tee -a "$INSTALL_LOG_FILE"
    fi
  else
