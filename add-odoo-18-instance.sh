...
OE_VERSION="18.0"  # Changed from 17.0 to 18.0
IS_ENTERPRISE="False"
...
if [[ "$SSL_CHOICE" =~ ^(yes|y)$ ]]; then
    ENABLE_SSL="True"
    # Prompt for domain name and www version
    read -p "Enter the domain name for the instance (e.g., mycompany.com): " WEBSITE_NAME
    read -p "Do you want to include www.$WEBSITE_NAME as well? (yes/no): " WWW_CHOICE
    read -p "Enter your email address for SSL certificate registration: " ADMIN_EMAIL

    # Validate domain name
    if [[ ! "$WEBSITE_NAME" =~ ^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$ ]]; then
        echo "Invalid domain name. Please enter a valid domain (e.g., example.com)"
        exit 1
    fi

    # Set up domain names for SSL
